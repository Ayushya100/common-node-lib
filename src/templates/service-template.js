...
class Service {
  constructor(
    ...
    this.payloadSizeLimit = process.env.PAYLOAD_SIZE_LIMIT; // Set payload size limit
    this.originPath = process.env.ORIGIN_PATH; // Set origin path
    this.urlPayloadLimit = process.env.URL_PAYLOAD_LIMIT; // Set URL payload limit
    this.apiLimit = Number(process.env.API_LIMIT) || 100; // Set API request limit
    this.windowSize = Number(process.env.WINDOW_SIZE) || 5 * 60 * 1000; // Set window size for rate limiting
    this.cookieKey = process.env.COOKIE_KEY; // Set the cookie encryption key

    this.cookieEnable = cookieEnable; // Enable or disable cookies based on input
    this.openApiEnabled = openApiEnabled; // Enable or disable OpenAPI support based on input
    this.serviceConfig = serviceConfig; // Store the service configuration object
    this.setUserContext = setUserContext; // Set the function used to attach user context to requests

    const mainModulePath = process.argv[1]; // Get the path of the main module being run
    const appRoot = path.dirname(mainModulePath); // Get the directory path of the main application
    this.openapiSpec = path.join(appRoot, 'openapi.yaml'); // Define the path to the OpenAPI spec file
